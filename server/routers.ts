  ...
  assistant: router({
    chat: protectedProcedure.input(z.object({ messages: z.array(z.object({ role: z.enum(["user", "assistant", "system"]), content: z.string() })).min(1), businessDate: z.string().optional() })).mutation(async ({ ctx, input }) => {
      ...
      const answer = await invokeLLM({
        messages: [{ role: "system", content: "You are an operations assistant. Understand natural language commands. Return JSON only. For roster commands, addLeader adds a Team Leader, addTester adds a tester under a Team Leader, deleteLeader deletes a Team Leader and their testers, deleteTester deletes a tester. For report rows, accept any tester name, optional Team Leader, project (Super X or Inception), and OTP/count. Convert one project value into the corresponding superX or inception number and leave the other as 0. A report row can be stored for the report preview even if the tester is not yet in the roster, but raw report data must never create or update Team Leader/tester roster records. Leave leader empty when the user omits it so the server can infer it from the existing tester roster. Only return addLeader/addTester actions when the user explicitly asks to add or create a roster member. " }, ...input.messages.map(message => ({ role: message.role as "user" | "assistant" | "system", content: message.content }))],
        response_format: { type: "json_schema", json_schema: { name: "assistant_command", strict: true, schema: { type: "object", properties: { answer: { type: "string" }, date: { type: "string" }, actions: { type: "array", items: { type: "object", properties: { type: { type: "string", enum: ["addLeader", "addTester", "deleteLeader", "deleteTester"] }, name: { type: "string" }, leader: { type: "string" } }, required: ["type", "name", "leader"], additionalProperties: false } }, rows: { type: "array", items: { type: "object", properties: { leader: { type: "string" }, tester: { type: "string" }, project: { type: "string", enum: ["Super X", "Inception"] }, quantity: { type: "number" } }, required: ["leader", "tester", "project", "quantity"], additionalProperties: false } }, notes: { type: "array", items: { type: "string" } } }, required: ["answer", "date", "actions", "rows", "notes"], additionalProperties: false } } },
        max_tokens: 4000,
      });
      const responseContent = answer.choices?.[0]?.message?.content;
      let extraction: { answer?: string; date?: string; actions?: Array<{ type: string; name: string; leader: string }>; rows: Array<{ leader: string; tester: string; project: "Super X" | "Inception"; quantity: number }>; notes?: string[] } = { rows: [] };
      ...
      if (db) {
        ...
        for (const action of extraction.actions ?? []) {
          ...
          if (action.type === "deleteLeader") { const leader = (await db.select().from(teamLeaders).where(eq(teamLeaders.name, name)).limit(1))[0]; if (leader) { const children = await db.select().from(testers).where(eq(testers.teamLeaderId, leader.id)); for (const child of children) await db.delete(dailyPerformance).where(eq(dailyPerformance.testerId, child.id)); await db.delete(testers).where(eq(testers.teamLeaderId, leader.id)); await db.delete(teamLeaders).where(eq(teamLeaders.id, leader.id)); } }
          if (action.type === "deleteTester") { const tester = (await db.select().from(testers).where(eq(testers.name, name)).limit(1))[0]; if (tester) { await db.delete(dailyPerformance).where(eq(dailyPerformance.testerId, tester.id)); await db.delete(testers).where(eq(testers.id, tester.id)); } }
        }
        ...
        const grouped = new Map<string, { leaderName: string; testerName: string; superX: number; inception: number; hasSuperX: boolean; hasInception: boolean; tester?: typeof roster[number]; leader?: typeof leaderRows[number] }>();
        for (const row of extraction.rows) {
          ...
          const item = grouped.get(key) ?? { leaderName, testerName: row.tester.trim(), superX: 0, inception: 0, hasSuperX: false, hasInception: false, tester, leader };
          if (row.project === "Super X") { item.superX += Number(row.quantity); item.hasSuperX = true; }
          if (row.project === "Inception") { item.inception += Number(row.quantity); item.hasInception = true; }
        ...
        for (const row of Array.from(grouped.values())) {
          if (!row.tester || !row.leader || row.tester.teamLeaderId !== row.leader.id) { exceptions.push(`Could not safely match ${row.testerName} under ${row.leaderName}; raw row kept in the report only.`); reportRows.push({ leader: row.leaderName, tester: row.testerName, superX: row.superX, inception: row.inception, total: row.superX + row.inception }); continue; }
          const projectValues = [{ key: "super x", quantity: row.superX, supplied: row.hasSuperX }, { key: "inception", quantity: row.inception, supplied: row.hasInception }];
          if (projectValues.some(item => item.supplied && (!projectByName.get(item.key) || !Number.isFinite(item.quantity) || item.quantity < 0))) { exceptions.push(`Invalid project values for ${row.testerName}`); continue; }
          for (const value of projectValues.filter(item => item.supplied)) {
            const project = projectByName.get(value.key)!;
            const existing = await db.select().from(dailyPerformance).where(and(eq(dailyPerformance.businessDate, toDate(reportDate)), eq(dailyPerformance.testerId, row.tester.id), eq(dailyPerformance.projectId, project.id))).limit(1);
            ...
            else await db.insert(dailyPerformance).values({ businessDate: toDate(reportDate), testerId: row.tester.id, teamLeaderId: row.leader.id, projectId: project.id, quantity: value.quantity.toString(), source: "AI assistant", notes: latest.slice(0, 1000) });
          }
          const saved = await db.select().from(dailyPerformance).where(and(eq(dailyPerformance.businessDate, toDate(reportDate)), eq(dailyPerformance.testerId, row.tester.id))).limit(20);
          const savedSuperX = saved.filter(item => projectByName.get("super x")?.id === item.projectId).reduce((sum, item) => sum + Number(item.quantity), 0);
          const savedInception = saved.filter(item => projectByName.get("inception")?.id === item.projectId).reduce((sum, item) => sum + Number(item.quantity), 0);
          stored += 1;
          reportRows.push({ leader: row.leader.name, tester: row.tester.name, superX: savedSuperX, inception: savedInception, total: savedSuperX + savedInception });
        }

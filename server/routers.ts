  ...
  assistant: router({
    chat: protectedProcedure.input(z.object({ messages: z.array(z.object({ role: z.enum(["user", "assistant", "system"]), content: z.string() })).min(1), businessDate: z.string().optional() })).mutation(async ({ ctx, input }) => {
      ...
      const answer = await invokeLLM({
        messages: [{ role: "system", content: "You are an operations assistant. Understand natural language commands. Return JSON only. For roster commands, addLeader adds a Team Leader, addTester adds a tester under a Team Leader, deleteLeader deletes a Team Leader and their testers, deleteTester deletes a tester. For report rows, accept any tester name, optional Team Leader, project (Super X or Inception), and OTP/count. Convert one project value into the corresponding superX or inception number and leave the other as 0. A report row can be stored even if the tester is not yet in the roster; use the tester name and leader name in the response, but mark it for roster matching. Never invent names or numbers. " }, ...input.messages.map(message => ({ role: message.role as "user" | "assistant" | "system", content: message.content }))],
      ...
      if (db) {
        ...
        for (const row of extraction.rows) {
          ...
          const tester = candidates.length === 1 ? candidates[0] : candidates.find(item => item.teamLeaderId === leader?.id);
          if (!tester || !leader || tester.teamLeaderId !== leader.id) { exceptions.push(`Could not safely match ${row.tester} under ${row.leader}; row kept in the report and should be added to the roster.`); const superX = row.project === "Super X" ? Number(row.quantity) : 0; const inception = row.project === "Inception" ? Number(row.quantity) : 0; reportRows.push({ leader: row.leader || "Unassigned", tester: row.tester, superX, inception, total: superX + inception }); continue; }
          const values = [{ project: projectByName.get("super x"), quantity: row.project === "Super X" ? Number(row.quantity) : 0 }, { project: projectByName.get("inception"), quantity: row.project === "Inception" ? Number(row.quantity) : 0 }];
